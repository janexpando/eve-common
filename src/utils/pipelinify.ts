...
export async function pipelinify(functions: (PipeFunction | number | Readable | Writable)[]) { // TODO tests
    let pipeFunctions = [];
    ...
    for (let i = 0; i < functions.length; i++) {
        ...
        if (i == 0) {
            if (functions[i] instanceof Readable)
                pipeFunctions.push(functions[i]);
            else
                pipeFunctions.push(new Readable({
                    objectMode: true,
                    async read() {
                        try {
            ...
            if (functions[i] instanceof Writable)
                pipeFunctions.push(functions[i]);
            else
                pipeFunctions.push(new Writable({
                    objectMode: true,
            ...
            pipeFunctions.push(new Transform({
                objectMode: true,
