...
test.serial(
    'mws function returns instance of the mws object for existing pair companyId and marketplace',
    async t => {
        ...
        nock(environment.COMPANY_SERVICE_URL).get(`/developer-config/${developerId}`).reply(200, {
            developerId,
            accessKey: 'accessKey',
            secretKey: 'secretKey',
            continent: 'europe'
        ...
        nock(environment.COMPANY_SERVICE_URL).get(`/company/${companyId}/marketplace/${marketplace}/mws-credentials`)
            .reply(200, {
                developerId,
                marketplaceId,
                sellerId,
