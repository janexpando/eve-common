...
export type AmazonOtherType = 'amazon_in' | 'amazon_cn';

export const AMAZON_OTHER: AmazonOtherType[] = ['amazon_in', 'amazon_cn'];
...
export function getContinent(marketplace: MarketplaceName): MwsRegion {
    if (AMAZON_EUROPE.includes(marketplace as AmazonEuropeType)) return 'europe';
    if (AMAZON_AMERICA.includes(marketplace as AmazonAmericaType)) return 'america';
    if (AMAZON_FAR_EAST.includes(marketplace as AmazonFarEast)) return 'far_east';
