...
export class OrderClient extends EveClient {
    ...
    async getOrdersByFulfillmentId(fulfillmentId: string): Promise<ApiOrder[]> {
        const response = await this.got.get(`${this.env.GATEWAY_URL}/orders?fulfillmentId=${fulfillmentId}`);
