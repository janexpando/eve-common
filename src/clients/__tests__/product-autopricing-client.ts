...
test.serial('modify product autopricing settings', async t => {
    ...
    nock(environment.PRODUCT_SERVICE_URL, { encodedQueryParams: true })
        .post('/company/5e3c16ef80039d91b45cf4cc/marketplace/amazon_de/product-autopricing/active', {
            skus: ['123'],
            active: true,
        })
        .reply(200, {}, [
...
test.serial('set product autopricing caps', async t => {
    // nock.recorder.rec();
    const companyId = new ObjectId('5e3c16ef80039d91b45cf4cc');
    const marketplace = 'amazon_de';
    const sku = '123';
    ...
    nock(environment.PRODUCT_SERVICE_URL, {"encodedQueryParams":true})
        .post('/company/5e3c16ef80039d91b45cf4cc/marketplace/amazon_de/product-autopricing/caps', {"skus":["123"],"topCap":4,"bottomCap":5})
        .reply(200, {}, [ 'X-DNS-Prefetch-Control',
            ...
            '2',
            'Date',
            'Fri, 07 Feb 2020 09:28:49 GMT',
            'Connection',
            'close' ]);


    const response = await client.setCaps(companyId, marketplace, [sku], 4, 5);
    t.is(response.statusCode, 200);
