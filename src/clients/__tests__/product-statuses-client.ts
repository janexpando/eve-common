...
function makeCreateProductStatusWithStatus(companyId: ObjectId) {
    return function createProductStatusWithStatus(
...
test.serial('get product stats summary', async t => {
    ...
    nock(`${env.PRODUCT_SERVICE_URL}`, { encodedQueryParams: true })
        .post('/company/5d5d3fe4ff54a60ef331dcbb/product-stats', {
            marketplaces: ['amazon_de', 'amazon_it'],
            hasStockFilter: true,
        })
        .reply(
            200,
    ...
    let productStatusesToBeCreated = [
        { status: 'ok', sku: 'SKU1', marketplace: marketplace1, hasStock: true },
        {
            status: 'ok',
            sku: 'SKU2',
    ...
    for (let product of productStatusesToBeCreated) {
        productStatuses.push(
            createProductStatusWithStatus(
                product.status as ProductStatusKind,
                product.sku,
                product.marketplace,
                product.hasStock,
            ),
