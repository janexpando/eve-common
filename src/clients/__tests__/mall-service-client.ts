...
import { ApiMallFulfillment, ApiMallFulfillmentResponse, MallServiceClient } from '../mall-service-client';
...
test.serial('fulfill mall order', async t => {
    ...
    nock(env.MALL_SERVICE_URL, { encodedQueryParams: true })
        .post('/company/5e1dcb382efb3a6fa7ea48ca/fulfillment', {
            fulfillment: {
                companyId: '5e1dcb382efb3a6fa7ea48ca',
                marketplaceOrderId: '12345',
                marketplace: 'mall_cz',
                trackingNumber: 'trackingNumber',
                trackingUrl: 'trackingUrl',
    ...
    const result: ApiMallFulfillmentResponse = await mallServiceClient.fulfillOrder(companyId, fulfillment);
